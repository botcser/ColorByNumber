	...
	public class Gif
	{
		...
		public static IEnumerable<GifFrame> DecodeIterator(byte[] bytes, FilterMode filterMode = FilterMode.Point)
		{
			...
			foreach (var frame in iterator)
			{
				...
				yield return new GifFrame(texture, frame.Texture.width, frame.Texture.height, frame.Delay);
			}
		}
		...
		/// </summary>
		public byte[] Encode(int scale = 1, MasterPalette palette = MasterPalette.DontApply)
		{
			var frames = Converter.ConvertFrames(Frames);

			frames.ForEach(i => i.ApplyPalette(palette));

			return new SimpleGif.Gif(frames).Encode(scale);
		}
		...
		public void EncodeParallel(Action<EncodeProgress> onProgress, int scale = 1, MasterPalette palette = MasterPalette.DontApply)
		{
			Converter.Convert(this).ApplyPalette(palette).EncodeParallel(onProgress, scale); // Threads are started here.
		}
	}
}